            ...
            <div class="row my-account-page">
                ...
                <div class="col-sm-9">
                    <div class="block-register-bds block-common">
                        <h2 class="title-style text-color2">Chỉnh sửa tin tức</h2>
                        <form id="frmNews" name="frmNews" class="frm-register-bds" action="{{ route('member.news.update', [$articleInfo->id]) }}" method="post">
                            @if (count($errors) > 0)
                            ...
                            <div class="form-row row">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select id="cate_id" name="cate_id" class="form-control select2">
                                            ...
                                            @foreach ($arrListCate as $cate)
                                                <option value="{{ $cate->id }}"{!! old('cate_id', $articleInfo->cate_id) == $cate->id ? ' selected="selected"' : '' !!}>{{ $cate->{'name_' . config('app.locale')} }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                <div class="col-md-12">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" id="title" name="title" value="{{ old('title', $articleInfo->{'title_' . config('app.locale')}) }}" placeholder="Tiêu đề">
                                    </div>
                                </div>
                                <div class="col-md-12">
                                    <div class="form-group">
                                        ...
                                        <textarea id="description" name="description" class="form-control editorBasic" rows="4" cols="80" placeholder="Mô tả tin tức...">{{ old('description', $articleInfo->{'description_' . config('app.locale')}) }}</textarea>
                                    </div>
                                </div>
                                <div class="col-md-12">
                                    <div class="form-group">
                                        ...
                                        <textarea id="content" name="content" class="form-control editorBasic" rows="8" cols="80" placeholder="Nội dung tin tức...">{{ old('content', $articleInfo->{'content_' . config('app.locale')}) }}</textarea>
                                    </div>
                                </div>
                                <div class="col-md-12">
                                    <div class="form-group">
                                            ...
                                            <div class="row">
                                                <div class="col-lg-6">
                                                    <div class="upload-photo-wrap" id="photo_panel" style="height: auto; line-height: normal; overflow: auto;">
                                                        @if (old('image_url', $articleInfo->image_url))
                                                            <div>
                                                                <img src="{{ image_url(old('image_url', $articleInfo->image_url)) }}" class="img-responsive" style="height: 90px;" />
                                                                <input type="hidden" name="image_url" value="{{ old('image_url', $articleInfo->image_url) }}">
                                                            </div>
                                                        @endif
                                                    </div>
                                                </div>
                            ...
                            <div class="form-row">
                                <div class="boxBtnSubmit">
                                    <div class="text-center">
                                        <button type="submit" class="btn btn-main btn-lg add_arrow">Lưu tin</button>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('#frmNews').on('submit', function() {
            if (confirm('Khi nhấn lưu bài viết này sẽ chuyển về tình trạng chờ duyệt, bạn có chắc là muốn lưu không?')) {
                return true;
            }
